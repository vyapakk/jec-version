/**
 * Chart components for Airbus Combined dashboard.
          ...
          <div className="h-[300px] sm:h-[350px] w-full -mx-2 sm:mx-0">
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={chartData} margin={{ top: 10, right: 30, left: 10, bottom: 0 }}>
                <defs><linearGradient id="gradient-airbus-trend" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="hsl(192, 95%, 55%)" stopOpacity={0.4} /><stop offset="95%" stopColor="hsl(192, 95%, 55%)" stopOpacity={0} /></linearGradient></defs>
...
// ═══════════════════════════════════════════════════════════════
// GroupedBarChart — side-by-side comparison of metrics by family
// ═══════════════════════════════════════════════════════════════
...
export function GroupedBarChart({ data, families, title, subtitle, downloadTitle }: GroupedBarChartProps) {
  ...
  const chartData = families
    .map(f => ({ family: f, Orders: data[f]?.orders || 0, Deliveries: data[f]?.deliveries || 0, Operational: data[f]?.operational || 0 }))
  ...
  return (
    <motion.div ref={chartRef} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="rounded-xl border border-border bg-card p-3 sm:p-6">
      ...
      <AnimatedViewSwitch view={view}
        chart={
          <div className="h-[300px] sm:h-[380px] w-full -mx-2 sm:mx-0">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData} margin={{ top: 10, right: 20, left: 10, bottom: 5 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="hsl(217, 33%, 18%)" />
                <XAxis dataKey="family" stroke="hsl(215, 20%, 55%)" fontSize={10} tickLine={false} angle={-20} textAnchor="end" height={50} />
                <YAxis stroke="hsl(215, 20%, 55%)" fontSize={10} tickLine={false} tickFormatter={v => v >= 1000 ? `${(v / 1000).toFixed(1)}K` : v} width={50} />
                <Tooltip content={<CustomTooltip />} />
                <Legend content={() => (
                  <div className="mt-3 flex flex-wrap justify-center gap-4">
                    {(["Orders", "Deliveries", "Operational"] as const).map(m => (
                      <div key={m} className="flex items-center gap-1.5">
                        <div className="h-3 w-3 rounded-sm" style={{ backgroundColor: barColors[m] }} />
                        <span className="text-xs sm:text-sm text-muted-foreground">{m}</span>
                      </div>
                    ))}
                  </div>
                )} />
                <Bar dataKey="Orders" fill={barColors.Orders} radius={[2, 2, 0, 0]} />
                <Bar dataKey="Deliveries" fill={barColors.Deliveries} radius={[2, 2, 0, 0]} />
                <Bar dataKey="Operational" fill={barColors.Operational} radius={[2, 2, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        }
...
// ═══════════════════════════════════════════════════════════════
// HorizontalBarChart — Top N airlines ranking
// ═══════════════════════════════════════════════════════════════
...
export function HorizontalBarChart({ data, title, subtitle, metricLabel, downloadTitle, barColor = CHART_COLORS[0] }: HorizontalBarChartProps) {
  ...
  return (
    <motion.div ref={chartRef} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="rounded-xl border border-border bg-card p-3 sm:p-6">
      ...
      <AnimatedViewSwitch view={view}
        chart={
          <div className="h-[500px] sm:h-[600px] w-full -mx-2 sm:mx-0">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={data} layout="vertical" margin={{ top: 5, right: 30, left: 10, bottom: 5 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="hsl(217, 33%, 18%)" horizontal={false} />
                <XAxis type="number" stroke="hsl(215, 20%, 55%)" fontSize={10} tickLine={false} tickFormatter={v => v >= 1000 ? `${(v / 1000).toFixed(1)}K` : v} />
                <YAxis type="category" dataKey="name" stroke="hsl(215, 20%, 55%)" fontSize={9} tickLine={false} width={120} />
                <Tooltip content={<CustomTooltip />} />
...
// ═══════════════════════════════════════════════════════════════
// SimpleDonutChart — for fleet tab (no year dimension)
// ═══════════════════════════════════════════════════════════════
...
export function SimpleDonutChart({ data: rawData, title, subtitle, metricLabel, downloadTitle }: SimpleDonutChartProps) {
  ...
  const pieData = useMemo(() => {
    ...
    if (sorted.length > 10) {
      const top = sorted.slice(0, 9).map((d, i) => ({ ...d, color: CHART_COLORS[i % CHART_COLORS.length] }));
      top.push({ name: "Others", value: sorted.slice(9).reduce((s, d) => s + d.value, 0), color: "hsl(215, 20%, 55%)" });
      return top;
    }
      ...
      <AnimatedViewSwitch view={view}
        chart={<>
          <div className="h-[280px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={pieData} cx="50%" cy="45%" innerRadius={50} outerRadius={85} paddingAngle={2} dataKey="value" nameKey="name" stroke="hsl(222, 47%, 6%)" strokeWidth={2} activeIndex={activeIndex ?? undefined} activeShape={renderActivePieShape} onMouseEnter={(_, i) => setActiveIndex(i)} onMouseLeave={() => setActiveIndex(null)}>
