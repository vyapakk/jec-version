    ...
    <div className="py-8">
      ...
      <div className="mb-8">
        <GroupedBarChart
          ...
          title="Orders vs Deliveries vs Operational by Aircraft Family"
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="mb-8 rounded-xl border border-border bg-card p-4 sm:p-6">
        <div className="mb-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <div>
            <h2 className="text-xl font-semibold text-foreground">Airline Fleet Overview</h2>
            <p className="text-sm text-muted-foreground">
              {filteredAirlines.length} airlines · Cumulative lifetime data
            </p>
          </div>
          <div className="relative w-full sm:w-[320px]">
            <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
            <Input placeholder="Search airline, region, or variant..." value={search} onChange={e => setSearch(e.target.value)} className="pl-10 bg-secondary/50 border-border text-foreground placeholder:text-muted-foreground" />
            {search && <button onClick={() => setSearch("")} className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground"><X className="h-4 w-4" /></button>}
          </div>
        </div>

        {filteredAirlines.length === 0 ? (
          <div className="py-12 text-center text-muted-foreground">No airlines found matching your search.</div>
        ) : (
          <>
            ...
            {filteredAirlines.length > 25 && (
              <div className="mt-4 flex justify-center">
                <Button variant="outline" size="sm" onClick={() => setShowAll(!showAll)} className="border-border text-muted-foreground hover:text-foreground">
                  {showAll ? <><ChevronUp className="mr-1 h-4 w-4" /> Show Less</> : <><ChevronDown className="mr-1 h-4 w-4" /> Show All {filteredAirlines.length} Airlines</>}
                </Button>
              </div>
            )}
          </>
