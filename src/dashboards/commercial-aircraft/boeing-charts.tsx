/**
 * Boeing chart components — internalized for commercial-aircraft dashboard.
 * Standalone copy of boeing-combined/charts.tsx.
 */
...
import { useChartDownload, type DrillDownState } from "./boeing-data";
      ...
      <AnimatedViewSwitch view={view}
        chart={
          <div className="h-[300px] sm:h-[350px] w-full -mx-2 sm:mx-0">
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={chartData} margin={{ top: 10, right: 30, left: 10, bottom: 0 }}>
                <defs><linearGradient id="gradient-boeing-trend" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="hsl(192, 95%, 55%)" stopOpacity={0.4} /><stop offset="95%" stopColor="hsl(192, 95%, 55%)" stopOpacity={0} /></linearGradient></defs>
                <CartesianGrid strokeDasharray="3 3" stroke="hsl(217, 33%, 18%)" />
                <XAxis dataKey="year" stroke="hsl(215, 20%, 55%)" fontSize={10} tickLine={false} interval={Math.ceil(years.length / 10)} />
...
export function MultiLineChart({ data, years, title, subtitle, segments, onSegmentClick, downloadTitle, gradientPrefix = "bml" }: MultiLineChartProps) {
  const chartRef = useRef<HTMLDivElement>(null);
  const { downloadChart } = useChartDownload();
  const [view, setView] = useState<"chart" | "table">("chart");
    ...
    <Dialog open={state.isOpen} onOpenChange={open => { if (!open) onClose(); }}>
      <DialogContent className="aircraft-interiors-theme max-w-3xl bg-card border-border text-foreground">
        ...
        <div className="h-[350px] w-full mt-4"><ResponsiveContainer width="100%" height="100%"><AreaChart data={filteredData} margin={{ top: 10, right: 30, left: 10, bottom: 0 }}>
          <defs><linearGradient id="gradient-drilldown-boeing-ca" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor={state.color} stopOpacity={0.4} /><stop offset="95%" stopColor={state.color} stopOpacity={0} /></linearGradient></defs>
          <CartesianGrid strokeDasharray="3 3" stroke="hsl(217, 33%, 18%)" />
          ...
          <Area type="monotone" dataKey="value" stroke={state.color} fill="url(#gradient-drilldown-boeing-ca)" strokeWidth={3} dot={{ fill: state.color, r: 3 }} activeDot={{ r: 6 }} />
        </AreaChart></ResponsiveContainer></div>
