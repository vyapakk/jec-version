/**
 * Fleet tab — Airbus, internalized for commercial-aircraft dashboard.
 */
...
import { useAirlineFleetData, type AirlineFleetSummary } from "./airbus-data";
import { KPICard } from "./ui-helpers";
import { GroupedBarChart, HorizontalBarChart, SimpleDonutChart } from "./airbus-charts";
import AppFooter from "./AppFooter";

export function AirbusFleetTab() {
  ...
  return (
    <div className="py-8">
      ...
      <div className="mb-8">
        <GroupedBarChart data={data.byFamily} families={data.families} title="Gross Orders vs Deliveries vs Operational by Aircraft Family" subtitle="Side-by-side comparison across aircraft families" downloadTitle="Airbus Fleet — By Aircraft Family" />
      </div>

      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }} className="mb-8">
        <h2 className="text-xl font-semibold text-foreground mb-4">Distribution Overview</h2>
        <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
          <SimpleDonutChart data={ordersByFamily} title="Gross Orders by Aircraft Family" subtitle="Lifetime cumulative" metricLabel="Gross Orders" downloadTitle="Airbus Fleet — Orders by Family" />
          <SimpleDonutChart data={deliveriesByRegion} title="Deliveries by Region" subtitle="Lifetime cumulative" metricLabel="Deliveries" downloadTitle="Airbus Fleet — Deliveries by Region" />
          <SimpleDonutChart data={operationalByRegion} title="Operational Fleet by Region" subtitle="Current snapshot" metricLabel="Operational" downloadTitle="Airbus Fleet — Operational by Region" />
        </div>
      </motion.div>

      <div className="mb-8">
        <HorizontalBarChart data={top20Airlines} title="Top 20 Airlines by Operational Fleet" subtitle="Largest operators by number of aircraft currently in service" metricLabel="Operational Aircraft" downloadTitle="Airbus Fleet — Top 20 Airlines" barColor="hsl(142, 71%, 45%)" />
      </div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="mb-8 rounded-xl border border-border bg-card p-4 sm:p-6">
        <div className="mb-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground">{filteredAirlines.length} customers · Cumulative lifetime data</p>
          </div>
