/**
 * Data parsing (XLSX) and hooks for Airbus — internalized standalone copy.
 */
...
export interface AirlineFleetRecord {
  customer: string; country: string; region: string; variant: string;
  metric: "Orders" | "Deliveries" | "Operational"; count: number;
}

export interface AirlineFleetVariantDetail {
  variant: string; orders: number; deliveries: number; operational: number;
}

export interface AirlineFleetSummary {
  name: string; country: string; region: string;
  totalOrders: number; totalDeliveries: number; operational: number; backlog: number;
  variants: AirlineFleetVariantDetail[];
}

export interface AirlineFleetData {
  totalOrders: number; totalDeliveries: number; totalOperational: number; totalBacklog: number;
  ...
  airlines: AirlineFleetSummary[]; families: string[]; regions: string[]; variants: string[];
}
...
function parseFleetExcel(ab: ArrayBuffer): AirlineFleetData {
  ...
  const airlines: AirlineFleetSummary[] = [...airlineMap.entries()]
    .map(([name, a]) => ({
      name, country: a.country, region: a.region,
      totalOrders: a.totalOrders, totalDeliveries: a.totalDeliveries, operational: a.operational,
...
// ═══════════════════════════════════════════════════════════════
// Summary Overview Types & Parsing
// ═══════════════════════════════════════════════════════════════

export interface AirbusSummaryVariant {
  variant: string; family: string; orders: number; deliveries: number; operational: number;
}

export interface AirbusSummaryOverviewData {
  totalOrders: number; totalDeliveries: number; totalInFleet: number;
  byFamily: Record<string, { orders: number; deliveries: number; operational: number }>;
  byVariant: AirbusSummaryVariant[]; families: string[];
}
