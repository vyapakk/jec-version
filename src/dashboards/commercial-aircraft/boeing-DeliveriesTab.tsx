/**
 * Boeing Deliveries tab — internalized for commercial-aircraft dashboard.
 */
...
import { useDeliveryData, useDrillDown, type DeliveryCustomerSummary } from "./boeing-data";
import { KPICard, YearRangeSelector, useYearRange, aggregateYears } from "./ui-helpers";
import { TrendLineChart, MultiLineChart, YearlyDonutChart, DrillDownModal } from "./boeing-charts";
import AppFooter from "./AppFooter";

export function BoeingDeliveriesTab() {
  ...
  return (
    <div className="py-8">
      ...
      <div className="mb-8"><MultiLineChart data={data.deliveriesByYearByModelFamily} years={filteredYears} title="Deliveries by Aircraft Model" subtitle="Delivery trends by model family" segments={data.modelFamilies} onSegmentClick={handleDrillDown} downloadTitle="Boeing Deliveries — By Aircraft Model" gradientPrefix="bdel-mf" /></div>
      <div className="mb-8"><MultiLineChart data={data.deliveriesByYearByRegion} years={filteredYears} title="Deliveries by Region" subtitle="Delivery trends by geographic region" segments={data.regions} onSegmentClick={handleDrillDown} downloadTitle="Boeing Deliveries — By Region" gradientPrefix="bdel-rg" /></div>

      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }} className="mb-8">
        <h2 className="text-xl font-semibold text-foreground mb-4">Distribution in {rangeLabel}</h2>
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-4">
          <YearlyDonutChart data={donutData.byRegion} year={donutYear} title="By Region" segments={regionsForDonut} metricLabel="Deliveries" onSegmentClick={handleDrillDown} downloadTitle={`Boeing Deliveries — By Region (${rangeLabel})`} yearLabel={rangeLabel} />
          <YearlyDonutChart data={donutData.byModel} year={donutYear} title="By Aircraft Model" segments={modelFamiliesForDonut} metricLabel="Deliveries" onSegmentClick={handleDrillDown} downloadTitle={`Boeing Deliveries — By Model (${rangeLabel})`} yearLabel={rangeLabel} />
          <YearlyDonutChart data={donutData.byEngine} year={donutYear} title="By Engine" segments={enginesForDonut} metricLabel="Deliveries" onSegmentClick={handleDrillDown} downloadTitle={`Boeing Deliveries — By Engine (${rangeLabel})`} yearLabel={rangeLabel} />
          <YearlyDonutChart data={donutData.byCountry} year={donutYear} title="By Country" segments={countriesForDonut} metricLabel="Deliveries" onSegmentClick={handleDrillDown} downloadTitle={`Boeing Deliveries — By Country (${rangeLabel})`} yearLabel={rangeLabel} />
        </div>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="mb-8 rounded-xl border border-border bg-card p-4 sm:p-6">
        <div className="mb-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <div><h2 className="text-xl font-semibold text-foreground">Customer Deliveries</h2><p className="text-sm text-muted-foreground">{filteredCustomers.length} customers found</p></div>
          ...
          {selectedCustomer && <>
            ...
            <div className="mt-4 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="rounded-lg bg-secondary/30 p-3"><p className="text-xs text-muted-foreground">Total Lifetime Deliveries</p><p className="text-lg font-bold text-foreground">{selectedCustomer.totalDeliveries.toLocaleString()}</p></div>
              ...
              <div><p className="text-xs text-muted-foreground mb-2">Delivery History</p><div className="overflow-x-auto max-h-[300px] overflow-y-auto relative"><table className="w-full"><thead className="sticky top-0 z-10"><tr className="border-b border-border bg-card"><th className="px-3 py-2 text-left text-xs font-medium text-muted-foreground">Year</th><th className="px-3 py-2 text-left text-xs font-medium text-muted-foreground">Model</th><th className="px-3 py-2 text-left text-xs font-medium text-muted-foreground">Engine</th><th className="px-3 py-2 text-right text-xs font-medium text-muted-foreground">Quantity</th></tr></thead><tbody className="divide-y divide-border">{selectedCustomer.deliveryDetails.map((od, i) => <tr key={i} className="hover:bg-secondary/20"><td className="px-3 py-2 text-sm text-foreground">{od.year}</td><td className="px-3 py-2 text-sm text-muted-foreground">{od.model}</td><td className="px-3 py-2 text-sm text-muted-foreground">{od.engine}</td><td className="px-3 py-2 text-sm text-right font-mono text-foreground">{od.quantity}</td></tr>)}</tbody></table></div></div>
            </div>
          </>}
        </DialogContent>
      </Dialog>

      <AppFooter sourceText="Boeing Deliveries Analysis" unitText="All values represent aircraft units delivered" />
    </div>
  );
}
