/**
 * UI helper components — standalone for commercial-aircraft dashboard.
 * Shared by both Airbus and Boeing sections.
 */
...
export function YearRangeSelector({ allYears, fromYear, toYear, onFromChange, onToChange }: {
  allYears: number[]; fromYear: number; toYear: number;
    ...
    <div className="flex flex-wrap items-center gap-2">
      ...
      <div className="flex items-center rounded-lg border border-border bg-secondary/50 p-0.5">
        {RANGE_SHORTCUTS.map(s => (
          <Button key={s.label} variant="ghost" size="sm" onClick={() => applyShortcut(s.span)}
      ...
      <div className="flex items-center gap-1.5">
        ...
        <Select value={fromYear.toString()} onValueChange={v => { const y = parseInt(v); onFromChange(y); if (y > toYear) onToChange(y); }}>
          <SelectTrigger className="w-[85px] h-8 text-xs border-border bg-secondary/50"><SelectValue /></SelectTrigger>
          <SelectContent className="bg-popover border-border max-h-[300px]">
            {[...allYears].reverse().map(y => (<SelectItem key={y} value={y.toString()} className="text-xs">{y}</SelectItem>))}
          </SelectContent>
        </Select>
