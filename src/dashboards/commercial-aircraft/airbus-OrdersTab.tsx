/**
 * Orders tab — Airbus, internalized for commercial-aircraft dashboard.
 */
...
import { useAirbusOrderData, useAirbusSummaryOverview, useDrillDown, type AirbusCustomerSummary } from "./airbus-data";
...
export function AirbusOrdersTab() {
  ...
  return (
    <div className="py-8">
      ...
      <div className="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-3 xl:grid-cols-5">
        ...
        <KPICard title="Total Customers (2021–2026)" value={totalCustomers} icon={Users} accentColor="chart-4" delay={0.3} />
      ...
      {/* Trend Line Chart */}
      <div className="mb-8">
        <TrendLineChart data={data.summary.grossByYear} years={filteredYears} title="Gross Order Trends" subtitle="Total Airbus gross orders over time" metricLabel="Gross Orders" downloadTitle="Airbus Orders — Gross Order Trends" />
      </div>

      {/* Multi-Line by Program */}
      <div className="mb-8">
        <MultiLineChart data={data.summary.grossByYearByProgram} years={filteredYears} title="Gross Orders by Aircraft Program" subtitle="Order trends by aircraft program" segments={data.summary.programs} onSegmentClick={handleDrillDown} downloadTitle="Airbus Orders — By Aircraft Program" gradientPrefix="ab-prog" />
      </div>

      {/* Net Orders Comparison */}
      <div className="mb-8">
        <MultiLineChart data={netComparisonData} years={filteredYears} title="Orders Comparison: Gross vs Net" subtitle="Gross orders vs net orders (year of cancellation) vs net orders (year of order)" segments={["Gross Orders", "Net (Year of Cancellation)", "Net (Year of Order)"]} downloadTitle="Airbus Orders — Gross vs Net Comparison" gradientPrefix="ab-net" />
      </div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }} className="mb-8">
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
          <YearlyDonutChart data={donutData.byProgram} year={donutYear} title="By Aircraft Program" segments={programsForDonut} metricLabel="Orders" onSegmentClick={handleProgramDonutDrillDown} downloadTitle={`Airbus Orders — By Program (${rangeLabel})`} yearLabel={rangeLabel} />
        </div>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="mb-8 rounded-xl border border-border bg-card p-4 sm:p-6">
        <div className="mb-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground">{filteredCustomers.length} customers found · Customer data from 2021–2026</p>
          </div>
