/**
 * Deliveries tab — Airbus, internalized for commercial-aircraft dashboard.
 */
...
import { airbusConfig as config } from "./airbus-config";
import { useAirbusDeliveryData, useAirbusSummaryOverview, useDrillDown, type AirbusDeliveryCustomerSummary } from "./airbus-data";
import { KPICard, YearRangeSelector, useYearRange, aggregateYears } from "./ui-helpers";
import { TrendLineChart, MultiLineChart, YearlyDonutChart, DrillDownModal } from "./airbus-charts";
import AppFooter from "./AppFooter";

export function AirbusDeliveriesTab() {
  ...
  return (
    <div className="py-8">
      ...
      <div className="mb-8">
        <TrendLineChart data={data.summary.deliveriesByYear} years={filteredYears} title="Delivery Trends" subtitle="Total Airbus deliveries over time" metricLabel="Deliveries" downloadTitle="Airbus Deliveries — Delivery Trends" />
      </div>

      <div className="mb-8">
        <MultiLineChart data={data.summary.deliveriesByYearByProgram} years={filteredYears} title="Deliveries by Aircraft Program" subtitle="Delivery trends by aircraft program" segments={data.summary.programs} onSegmentClick={handleDrillDown} downloadTitle="Airbus Deliveries — By Aircraft Program" gradientPrefix="ab-del-prog" />
      </div>

      {detailFilteredYears.length > 0 && (
        <div className="mb-8">
          <MultiLineChart data={data.deliveriesByYearByRegion} years={detailFilteredYears} title="Deliveries by Region" subtitle={`Regional delivery trends (${detailYearRange} data available)`} segments={data.regions} onSegmentClick={(name, yearlyData, color) => { openDrillDown(name, data.deliveriesByYearByRegion[name] || {}, color, "Deliveries"); }} downloadTitle="Airbus Deliveries — By Region" gradientPrefix="ab-del-reg" />
        </div>
      )}
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }} className="mb-8">
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
          <YearlyDonutChart data={donutProgramData} year={donutYear} title="By Aircraft Program" segments={programsForDonut} metricLabel="Deliveries" onSegmentClick={handleProgramDonutDrillDown} downloadTitle={`Airbus Deliveries — By Program (${rangeLabel})`} yearLabel={rangeLabel} />
          {regionsForDonut.length > 0 && (
            <YearlyDonutChart data={donutRegionData} year={donutYear} title="By Region" segments={regionsForDonut} metricLabel="Deliveries" onSegmentClick={handleRegionDonutDrillDown} downloadTitle={`Airbus Deliveries — By Region (${rangeLabel})`} yearLabel={rangeLabel} />
          )}
        </div>
        {detailFilteredYears.length === 0 && (
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="mb-8 rounded-xl border border-border bg-card p-4 sm:p-6">
        <div className="mb-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground">{filteredCustomers.length} customers found · Customer data from {detailYearRange}</p>
          </div>
        ...
        {filteredCustomers.length === 0 ? (
          <div className="py-12 text-center text-muted-foreground"><p>No customer data available.</p></div>
        ) : (
          <>
