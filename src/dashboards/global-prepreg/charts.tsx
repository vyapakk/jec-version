          ...
          <div className="h-[300px] sm:h-[350px] w-full -mx-2 sm:mx-0">
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={chartData} margin={{ top: 10, right: 30, left: 10, bottom: 0 }}>
                <defs>
                  <linearGradient id="gradient-market-size-gp" x1="0" y1="0" x2="0" y2="1">
                ...
                <AreaChart data={chartData} margin={{ top: 10, right: 15, left: 5, bottom: 0 }}>
                  <defs>
                    {showSegments && segments ? segments.map((seg, idx) => (
                      <linearGradient key={seg.name} id={`gp-gradient-${idx}`} x1="0" y1="0" x2="0" y2="1">
                      ...
                      <linearGradient id="gp-gradient-total" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="5%" stopColor="hsl(192, 95%, 55%)" stopOpacity={0.4} />
                        <stop offset="95%" stopColor="hsl(192, 95%, 55%)" stopOpacity={0} />
                      </linearGradient>
                    )}
                  ...
                  {showSegments && segments ? (
                    <>
                      ...
                      {segments.map((seg, idx) => (
                        <Area key={seg.name} type="monotone" dataKey={seg.name} stroke={CHART_COLORS[idx % CHART_COLORS.length]} fill={`url(#gp-gradient-${idx})`} strokeWidth={2} style={{ cursor: "pointer" }} />
                      ))}
                    </>
                  ) : (
                    <Area type="monotone" dataKey="total" stroke="hsl(192, 95%, 55%)" fill="url(#gp-gradient-total)" strokeWidth={2} name="Market Size" />
                  )}
                </AreaChart>
              </ResponsiveContainer>
            </div>
...
export function DrillDownModal({ isOpen, onClose, segmentName, segmentData, color, useMillions = false }: DrillDownModalProps) {
  ...
  const { downloadChart } = useChartDownload();
  const unit = useUnit();
  const gradientId = `gp-drillGradient-${segmentName.replace(/[^a-zA-Z0-9]/g, "-")}`;
