/**
 * Types, data fetching hook, utility hooks, and UnitContext for this dashboard.
 * Supports dual value/weight datasets with a unit toggle.
 */
...
export interface UnitConfig {
  prefix: string;   // "$" for value, "" for weight
  suffix: string;   // "M" for value, " M Lbs" for weight
  decimals: number;  // 1 for value, 2 for weight
  label: string;    // "US$ Million" or "Million Lbs"
}

export const VALUE_UNIT: UnitConfig = { prefix: "$", suffix: "M", decimals: 1, label: "US$ Million" };
export const WEIGHT_UNIT: UnitConfig = { prefix: "", suffix: " M Lbs", decimals: 2, label: "Million Lbs" };

export const UnitContext = createContext<UnitConfig>(VALUE_UNIT);
export const useUnit = () => useContext(UnitContext);

/** Format a numeric value according to the current unit config */
export function fmtVal(value: number, unit: UnitConfig): string {
  const formatted = value.toLocaleString(undefined, {
    minimumFractionDigits: unit.decimals,
    maximumFractionDigits: unit.decimals,
  });
  return `${unit.prefix}${formatted}${unit.suffix}`;
}

/** Format for axis ticks (shorter) */
export function fmtAxis(value: number, unit: UnitConfig): string {
  if (unit.prefix === "$") return `$${Math.round(value)}M`;
  return `${value.toFixed(unit.decimals)} M Lbs`;
}
...
interface CompactMarketDataInner {
  totalMarket: number[];
...
interface DualCompactData {
  years: number[];
  value: CompactMarketDataInner;
  weight: CompactMarketDataInner;
}
...
function expandSegment(years: number[], segment: Record<string, number[]>): SegmentData[] {
  return Object.entries(segment).map(([name, values]) => ({ name, data: expandValues(years, values) }));
}
...
function expandInner(years: number[], compact: CompactMarketDataInner): MarketData {
  return {
    years,
...
interface UseDualMarketDataResult {
  valueData: MarketData | null;
  weightData: MarketData | null;
  isLoading: boolean;
  error: string | null;
  refetch: () => void;
}

export function useMarketData(dataUrl: string): UseDualMarketDataResult {
  const [valueData, setValueData] = useState<MarketData | null>(null);
  const [weightData, setWeightData] = useState<MarketData | null>(null);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const dual: DualCompactData = await response.json();
      const { years } = dual;
      setValueData(expandInner(years, dual.value));
      setWeightData(expandInner(years, dual.weight));
    } catch (err) {
  ...
  return { valueData, weightData, isLoading, error, refetch: fetchData };
}
