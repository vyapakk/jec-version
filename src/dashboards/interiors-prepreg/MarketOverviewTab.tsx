/**
 * Market Overview Tab — Aircraft Interiors Prepreg with unit-aware formatting.
 */

import { DollarSign, TrendingUp, BarChart3 } from "lucide-react";
import { KPICard } from "./ui-helpers";
import { MarketOverviewChart, DistributionDonutsRow, DrillDownModal } from "./charts";
import { MarketData, calculateCAGR, SegmentData, YearlyData, useDrillDown, useUnit } from "./data";
...
export function MarketOverviewTab({ marketData, selectedYear, onNavigateToTab }: MarketOverviewTabProps) {
  ...
  const handleSliceClick = (segmentName: string, segmentData: YearlyData[], color: string, donutType: TabType) => {
    ...
    switch (donutType) {
      ...
      case "aircraft": relatedSegments = { title: `By ${labels.application}`, data: marketData.application }; break;
      case "region": relatedSegments = { title: `By ${labels.endUser}`, data: marketData.endUser }; break;
      case "application": relatedSegments = { title: "By Resin Type", data: marketData.aircraftType }; break;
      ...
      case "material": relatedSegments = { title: "By Region", data: marketData.region }; break;
    }
  ...
  return (
    <div className="space-y-8">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <KPICard title={`${selectedYear} Market Size`} value={currentMarketValue} prefix={unit.prefix} suffix={unit.suffix} decimals={unit.decimals} icon={DollarSign} delay={0} accentColor="primary" />
        <KPICard title={`CAGR (${firstYear}-${lastYear})`} value={cagrValue} prefix="" suffix="%" icon={BarChart3} delay={0.1} accentColor="chart-4" />
        <KPICard title={`${lastYear} Forecast`} value={valueLast} prefix={unit.prefix} suffix={unit.suffix} decimals={unit.decimals} icon={TrendingUp} delay={0.2} accentColor="accent" />
      </div>
      ...
      <div>
        ...
        <DistributionDonutsRow
          ...
          materialTypeLabel={labels.materialType}
        />
      </div>
