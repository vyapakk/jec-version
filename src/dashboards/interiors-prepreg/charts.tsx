...
export function DistributionDonutsRow({ endUserData, aircraftData, regionData, applicationData, equipmentData, processTypeData, materialTypeData, year, onDonutClick, onSliceClick, endUserLabel = "End User", equipmentLabel = "Equipment", processTypeLabel = "Process Type", applicationLabel = "Application", materialTypeLabel = "OEM" }: DistributionDonutsRowProps) {
  ...
  const hasMaterialType = materialTypeData && materialTypeData.length > 0;
  const donutCount = 1 + (hasAircraft ? 1 : 0) + 1 + (hasApplication ? 1 : 0) + 1 + (hasProcessType ? 1 : 0) + (hasMaterialType ? 1 : 0);
  const gridCols = donutCount >= 7 ? "grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-7" : donutCount >= 6 ? "grid-cols-2 sm:grid-cols-3 lg:grid-cols-6" : donutCount === 5 ? "grid-cols-2 sm:grid-cols-3 lg:grid-cols-5" : "grid-cols-2 sm:grid-cols-3 lg:grid-cols-4";
  ...
  return (
    <div className={`grid ${gridCols} gap-4`}>
      ...
      <MiniDonut data={equipmentData} year={year} title={equipmentLabel} tabType="equipment" onClick={onDonutClick} onSliceClick={onSliceClick} delay={0.25} />
      {hasProcessType && <MiniDonut data={processTypeData} year={year} title={processTypeLabel} tabType="process" onClick={onDonutClick} onSliceClick={onSliceClick} delay={0.3} />}
      {hasMaterialType && <MiniDonut data={materialTypeData} year={year} title={materialTypeLabel} tabType="material" onClick={onDonutClick} onSliceClick={onSliceClick} delay={0.35} />}
      <MiniDonut data={regionData} year={year} title="Region" tabType="region" onClick={onDonutClick} onSliceClick={onSliceClick} delay={0.4} />
    </div>
  );
}
          ...
          <tbody className="divide-y divide-border">
            {tableData.map((row, idx) => (
              <motion.tr key={row.name} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.6 + idx * 0.05 }}
                ...
                <td className="px-3 sm:px-6 py-3 sm:py-4 text-right font-mono text-sm text-muted-foreground whitespace-nowrap">{fmtVal(row.startValue, unit)}</td>
                <td className="px-3 sm:px-6 py-3 sm:py-4 text-right font-mono text-sm text-foreground whitespace-nowrap">{fmtVal(row.endValue, unit)}</td>
