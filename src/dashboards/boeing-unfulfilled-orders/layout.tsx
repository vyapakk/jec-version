/**
 * Layout components for Boeing Unfulfilled Orders: Header, Skeleton, ScrollToTop.
 * Fully standalone.
 */
...
export function DashboardHeader({ title = "Boeing Unfulfilled Orders", subtitle = "Current Backlog of Pending Aircraft Orders" }: { title?: string; subtitle?: string }) {
  return (
      ...
      <div className="container relative mx-auto px-4 py-6 md:py-8">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 }} className="flex items-center gap-2 rounded-lg bg-secondary/50 px-4 py-2 border border-border">
            ...
            <span className="text-sm text-muted-foreground">Snapshot: Current Backlog</span>
          </motion.div>
        </div>
      </div>
    </motion.header>
