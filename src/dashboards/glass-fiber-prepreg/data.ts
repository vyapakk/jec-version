/**
 * Dual-unit data layer for Glass Fiber Prepreg Market.
 * Supports Value (US$ Million) and Volume (Million Lbs) toggle.
 */
...
export function fmtAxis(value: number, unit: UnitConfig): string {
  ...
  return `${value.toFixed(unit.decimals)}`;
}
...
function expandUnit(years: number[], compact: CompactUnit): MarketData {
  return {
    years,
...
interface UseDualMarketDataResult {
  ...
  volumeData: MarketData | null;
  isLoading: boolean;
  error: string | null;
  refetch: () => void;
}

export function useDualMarketData(dataUrl: string): UseDualMarketDataResult {
  const [valueData, setValueData] = useState<MarketData | null>(null);
  const [volumeData, setVolumeData] = useState<MarketData | null>(null);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const compact: CompactDualData = await response.json();
      const { years } = compact;
      setValueData(expandUnit(years, compact.value));
      setVolumeData(expandUnit(years, compact.volume));
    } catch (err) {
  ...
  return { valueData, volumeData, isLoading, error, refetch: fetchData };
}
