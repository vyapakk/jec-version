/**
 * Main Dashboard — Boeing Deliveries.
...
import { useDeliveryData, useDrillDown, type CustomerSummary } from "./data";
import { DashboardHeader, DashboardSkeleton, ScrollToTop, YearSelector } from "./layout";
import { KPICard } from "./ui-helpers";
import { DeliveryTrendLineChart, MultiLineChart, DeliveryDonutChart, DrillDownModal } from "./charts";

const BoeingDeliveriesDashboard = () => {
  const navigate = useNavigate();
  const [selectedYear, setSelectedYear] = useState<number>(config.defaultYear);
  const { data, isLoading, error, refetch } = useDeliveryData(config.dataUrl);
  ...
  const modelFamiliesForDonut = useMemo(() => {
    ...
    return data.modelFamilies
      .filter((mf) => (data.deliveriesByYearByModelFamily[mf]?.[selectedYear] || 0) > 0)
      .sort((a, b) => (data.deliveriesByYearByModelFamily[b]?.[selectedYear] || 0) - (data.deliveriesByYearByModelFamily[a]?.[selectedYear] || 0));
  }, [data, selectedYear]);

  const regionsForDonut = useMemo(() => {
    if (!data) return [];
    return data.regions
      .filter((r) => (data.deliveriesByYearByRegion[r]?.[selectedYear] || 0) > 0)
      .sort((a, b) => (data.deliveriesByYearByRegion[b]?.[selectedYear] || 0) - (data.deliveriesByYearByRegion[a]?.[selectedYear] || 0));
  }, [data, selectedYear]);

  const enginesForDonut = useMemo(() => {
    if (!data) return [];
    return data.engines
      .filter((e) => (data.deliveriesByYearByEngine[e]?.[selectedYear] || 0) > 0)
      .sort((a, b) => (data.deliveriesByYearByEngine[b]?.[selectedYear] || 0) - (data.deliveriesByYearByEngine[a]?.[selectedYear] || 0));
  }, [data, selectedYear]);

  const countriesForDonut = useMemo(() => {
    if (!data) return [];
    return Object.keys(data.deliveriesByYearByCountry)
      .filter((c) => (data.deliveriesByYearByCountry[c]?.[selectedYear] || 0) > 0)
      .sort((a, b) => (data.deliveriesByYearByCountry[b]?.[selectedYear] || 0) - (data.deliveriesByYearByCountry[a]?.[selectedYear] || 0))
      .slice(0, 20);
  }, [data, selectedYear]);
  ...
  const yearCustomers = useMemo(() => {
    if (!data) return 0;
    return new Set(data.deliveries.filter((o) => o.year === selectedYear).map((o) => o.customer)).size;
  }, [data, selectedYear]);
  ...
  return (
    <div className="aircraft-interiors-theme min-h-screen bg-background text-foreground">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        {/* Line Charts */}
        <div className="mb-8">
          <DeliveryTrendLineChart data={data.deliveriesByYear} years={data.years} title="Delivery Trends" subtitle="Total Boeing deliveries over time" />
        </div>

        <div className="mb-8">
          <MultiLineChart
            data={data.deliveriesByYearByModelFamily} years={data.years}
            title="Deliveries by Aircraft Model" subtitle="Delivery trends by model family"
            segments={data.modelFamilies} onSegmentClick={openDrillDown}
          />
        </div>
        <div className="mb-8">
          <MultiLineChart
            data={data.deliveriesByYearByRegion} years={data.years}
            title="Deliveries by Region" subtitle="Delivery trends by geographic region"
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }} className="mb-8">
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-4">
            <DeliveryDonutChart data={data.deliveriesByYearByRegion} year={selectedYear} title="By Region" segments={regionsForDonut} onSegmentClick={openDrillDown} />
            <DeliveryDonutChart data={data.deliveriesByYearByModelFamily} year={selectedYear} title="By Aircraft Model" segments={modelFamiliesForDonut} onSegmentClick={openDrillDown} />
            <DeliveryDonutChart data={data.deliveriesByYearByEngine} year={selectedYear} title="By Engine" segments={enginesForDonut} onSegmentClick={openDrillDown} />
            <DeliveryDonutChart data={data.deliveriesByYearByCountry} year={selectedYear} title="By Country" segments={countriesForDonut} onSegmentClick={openDrillDown} />
          </div>
        </motion.div>
                ...
                <div className="mt-4 space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="rounded-lg bg-secondary/30 p-3">
                      ...
                      <p className="text-lg font-bold text-foreground">{selectedCustomer.totalDeliveries.toLocaleString()}</p>
                    </div>
                    <div className="rounded-lg bg-secondary/30 p-3">
                  ...
                  <div>
                    <p className="text-xs text-muted-foreground mb-2">Aircraft Models Delivered</p>
                  ...
                  <div>
                    <p className="text-xs text-muted-foreground mb-2">Delivery History</p>
                    <div className="overflow-x-auto max-h-[300px] overflow-y-auto relative">
                      <table className="w-full">
                        ...
                        <tbody className="divide-y divide-border">
                          {selectedCustomer.deliveryDetails.map((od, i) => (
        ...
        <AppFooter variant="dark" sourceText="Boeing Deliveries Analysis" unitText="All values represent aircraft units delivered" />
      </main>
    </div>
  );
};

export default BoeingDeliveriesDashboard;
