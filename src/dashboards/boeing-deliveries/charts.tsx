/**
 * Chart components for Boeing Deliveries dashboard.
 * DeliveryTrendLineChart (with YoY), MultiLineChart, DeliveryDonutChart, DrillDownModal.
 */
...
// ── DeliveryTrendLineChart (with YoY growth) ──────────────────
...
export function DeliveryTrendLineChart({ data, years, title, subtitle }: DeliveryTrendLineChartProps) {
  const chartRef = useRef<HTMLDivElement>(null);
  const { downloadChart } = useChartDownload();
  const [view, setView] = useState<"chart" | "table">("chart");

  const chartData = years.map((year, index) => {
    const deliveries = data[year] || 0;
    const prevYear = index > 0 ? years[index - 1] : null;
    const prevDeliveries = prevYear !== null ? (data[prevYear] || 0) : null;
    const yoyGrowth = prevDeliveries !== null && prevDeliveries > 0 ? ((deliveries - prevDeliveries) / prevDeliveries) * 100 : null;
    return { year, deliveries, yoyGrowth };
  });

  const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload?.length) {
      const delEntry = payload.find((p: any) => p.dataKey === "deliveries");
        ...
        <div className="rounded-lg border border-border bg-popover p-4 shadow-lg">
          ...
          {delEntry && (
            <div className="flex items-center gap-2 text-sm">
              ...
              <span className="font-mono font-medium text-foreground">{delEntry.value.toLocaleString()}</span>
            </div>
          )}
  ...
  return (
    <motion.div ref={chartRef} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="rounded-xl border border-border bg-card p-3 sm:p-6">
      ...
      <AnimatedViewSwitch view={view}
          ...
          <div className="h-[300px] sm:h-[350px] w-full -mx-2 sm:mx-0">
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={chartData} margin={{ top: 10, right: 30, left: 10, bottom: 0 }}>
                <defs>
                  <linearGradient id="gradient-deliveries" x1="0" y1="0" x2="0" y2="1">
                ...
                <Line yAxisId="left" type="monotone" dataKey="deliveries" stroke="hsl(192, 95%, 55%)" strokeWidth={3} dot={{ fill: "hsl(192, 95%, 55%)", strokeWidth: 0, r: 4 }} activeDot={{ r: 6, strokeWidth: 0 }} name="Deliveries" />
        ...
        table={<DataTable headers={["Year", "Deliveries", "YoY Growth (%)"]} rows={chartData.map((d) => [d.year, d.deliveries.toLocaleString(), d.yoyGrowth !== null ? `${d.yoyGrowth >= 0 ? "+" : ""}${d.yoyGrowth.toFixed(1)}%` : "—"])} />}
      />
    </motion.div>
  );
}
                ...
                <AreaChart data={chartData} margin={{ top: 10, right: 15, left: 5, bottom: 0 }}>
                  <defs>
                    {segments.map((seg, idx) => (
                      <linearGradient key={seg} id={`gradient-del-${idx}`} x1="0" y1="0" x2="0" y2="1">
                  ...
                  <Legend content={renderLegend} />
                  {segments.map((seg, idx) => (
                    <Area key={seg} type="monotone" dataKey={seg} stroke={CHART_COLORS[idx % CHART_COLORS.length]} fill={`url(#gradient-del-${idx})`} strokeWidth={2} style={{ cursor: "pointer" }} />
                  ))}
                </AreaChart>
              </ResponsiveContainer>
            </div>
...
// ── DeliveryDonutChart (matching old style) ───────────────────
...
interface DeliveryDonutChartProps {
  data: Record<string, Record<number, number>>;
  year: number;
  title: string;
  segments: string[];
...
export function DeliveryDonutChart({ data, year, title, segments, onSegmentClick }: DeliveryDonutChartProps) {
      ...
      return (
        <div className="rounded-lg border border-border bg-popover p-4 shadow-lg">
          ...
          <div className="mt-1 space-y-1 text-sm">
            <p className="text-muted-foreground">Deliveries: <span className="font-mono font-medium text-foreground">{d.value.toLocaleString()}</span></p>
  ...
  return (
    <motion.div ref={chartRef} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5 }} className="rounded-xl border border-border bg-card p-3 sm:p-6">
      ...
      <AnimatedViewSwitch view={view}
        ...
        table={
          <DataTable
            headers={["Segment", "Deliveries", "Share"]}
...
export function DrillDownModal({ state, years, onClose }: DrillDownModalProps) {
  ...
  const filteredData = years.filter((y) => y >= minYear && y <= maxYear).map((year) => ({ year, deliveries: state.yearlyData[year] || 0 }));
      ...
      <DialogContent className="aircraft-interiors-theme max-w-3xl bg-card border-border text-foreground">
        <DialogHeader>
          <DialogTitle className="text-foreground flex items-center gap-2">
            ...
            {state.segmentName} — Delivery Trend
          </DialogTitle>
        </DialogHeader>
        <div className="h-[350px] w-full mt-4">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={filteredData} margin={{ top: 10, right: 30, left: 10, bottom: 0 }}>
              <defs>
                <linearGradient id="gradient-drilldown-del" x1="0" y1="0" x2="0" y2="1">
              ...
              <Area type="monotone" dataKey="deliveries" stroke={state.color} fill="url(#gradient-drilldown-del)" strokeWidth={3} dot={{ fill: state.color, r: 3 }} activeDot={{ r: 6 }} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
        <div className="mt-2 max-h-[200px] overflow-y-auto">
          <DataTable
            headers={["Year", "Deliveries"]}
            rows={filteredData.map((d) => [d.year, d.deliveries.toLocaleString()])}
          />
        </div>
      </DialogContent>
    </Dialog>
